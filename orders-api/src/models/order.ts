...
export class OrderModel {
  static async create(data: CreateOrderData): Promise<OrderWithItems> {
    ...
    try {
      ...
      let totalCents = 0;
      const orderItems: Array<{ product_id: number; qty: number; unit_price_cents: number; subtotal_cents: number }> = [];
      ...
      for (const item of data.items) {
        ...
        if (product.stock < item.qty) {
          throw new AppError(400, `Insufficient stock for product ID ${item.product_id}. Available: ${product.stock}, requested: ${item.qty}`);
        }
